...
tsv_dir = 'C:/Users/Skyvein/Desktop/TMB/code'
df = pd.DataFrame(pd.read_csv(tsv_dir + '/TCGA-COAD.muse_snv.tsv', sep='	'))

qt_dir = 'D:/coad/filtered'
...
# 得到前#个病例
highTMB = set(e[0] for e in arr[:63]) # the number of high TMB cases are calculated from R script
lowTMB = set(e[0] for e in arr[63:126])
...
for dn in os.listdir(qt_dir):
    if dn.find('TCGA-') == 0:
        ...
        if sampleDic.__contains__(sample) and dn.find('-bad') < 0:
            ...
            for dn2 in slices:
                if dn2[-3:] == 'tif' and (dn2.find('-20X') >= 0 or dn2.find('-20x') >= 0):
                    src = qt_dir + '/' + dn + '/' + dn2
